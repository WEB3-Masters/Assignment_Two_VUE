<template>
    <div class="gameplay">
        ...
        <div class="decks">         
            <div class="card"><button @click="drawCard">Draw Card {{ drawPileSize }}</button></div>
            <div class="discard">{{ topDiscardCard?.type }} {{ topDiscardCard?.color}} {{ topDiscardCard?.number}}</div>
        </div>
        ...
        <div class="playerhand" ref="cardsContainer">
            ...
            <div class = "cards">
                <div v-for="card,index in playerHand" :key ="index" :class="['card',{playable: isPlayable(index)}]">
...
import {createGame, Game} from '../../../Assignment_One/Assignment-One-Remake/src/model/uno';
...
const startGame = () => {
    ...
    message += ("First card is: " + topDiscardCard.value?.type + " " + topDiscardCard.value?.color + " " + topDiscardCard.value?.number)
    console.log("First card is: " + topDiscardCard.value?.type + " " + topDiscardCard.value?.color + " " + topDiscardCard.value?.number);
};

const playerHand = computed(()=>game.value?.hand?.playerHand(currentPlayer.value)?? []);
...
const playCard = (cardIndex: number) => {
    try {
        ...
        message = "";
        message += ("Player " + (currentPlayer.value +1) + " played: " + card?.type + " " + card?.color + " " + card?.number);
        console.log("Player " + (currentPlayer.value) + " played: " + card?.type + " " + card?.color + " " + card?.number);
